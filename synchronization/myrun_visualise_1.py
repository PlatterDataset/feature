...
with open(os.path.join(opt.work_dir,opt.reference,'offsets.pckl'), 'rb') as fil:
	offsets = pickle.load(fil, encoding='latin1')
...
for tidx, track in enumerate(tracks):
	...
	offset = offsets[tidx]
	offset = numpy.pad(offset, (3, 3), 'constant', constant_values=0)
	#我真的不知道啥情况
	print(len(offset))
	for fidx, frame in enumerate(track['track']['frame'].tolist()) :
		faces[frame].append({'track': tidx, 'conf':fconfm[fidx], 's':track['proc_track']['s'][fidx], 'x':track['proc_track']['x'][fidx], 'y':track['proc_track']['y'][fidx], 'offset':offset[fidx]})
...
for fidx, fname in enumerate(flist):
	...
	for face in faces[fidx]:
		...
		cv2.putText(image,'Track %d, Conf %.3f, Offset %f'%(face['track'],face['conf'],face['offset']), (int(face['x']-face['s']),int(face['y']-face['s'])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),2)
